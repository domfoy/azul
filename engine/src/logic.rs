...
use super::game::MAX_PENALTY_COUNT;
use super::elements::{
    Action,
    Board,
    ColourGroup,
...
fn get_pick(game: &mut Game, action: &Action) -> Pick {
    match action.picked_place {
        PickedPlace::Center => game.round.table.center.pick_colour(action.colour),
        PickedPlace::Factory(factoryId) => game.round.table.factories[factoryId].pick_colour(action.colour)
    }
}

pub fn add_action(game: &mut Game, action: &Action) {
    let pick = get_pick(game, action);

    add_pick_to_player_board(game, action, pick)
}
...
//         },
//         PickedPlace::Factory(factory_id) => {
...
pub fn prepare_round(game: &mut Game) -> Round {
    let mut round = Round::new(game.players.len());
    for factory_id in 0..game.round.table.factories.len() {
        let colour_groups = game.bag.pop(4);

        round.table.factories[factory_id] = Factory(colour_groups);
    }
